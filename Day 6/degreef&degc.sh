...
function fahrenheit() {
	...
	then
		#using 'x' variable just for simplify the conversion
		#x=temp*9/5
...
function celsius() {
	read -p "Enter a temprature in Fahrenheit : " temp
	if [ $temp -ge 32 -a $temp -le 212 ]
	then
		#usimg 'x' variable just for simplify the calculation
		x=$((temp-32))
		#x=temp-32
...
case "$conversion" in
	...
	2)
		...
		;;
	*)
		echo "Enter proper conversionmode."
		;;
esac
